...
navigationTabsInit(resumeListItems, 'resume__list-item--active', 'resume__content--show');
navigationTabsInit(PortFolioListItems, 'portfolio__item--active', 'portfolio-content--show');

function navigationTabsInit (listItems, listItemActiveClass, contentItemShowClass){
    listItems.forEach(item => {
        item.addEventListener('click', ()=>{

            removeAvtiveClass(listItemActiveClass);

            item.classList.add(listItemActiveClass);
    
            const removeActiveItem = document.querySelector(`.${contentItemShowClass}`);
            removeActiveItem.classList.remove(contentItemShowClass);
    
            const showContent = document.getElementById(Object.entries(item.dataset)[0][1]);
            showContent.classList.add(contentItemShowClass);
...
function removeAvtiveClass (className){
    const itemActive = document.querySelector(`.${className}`);
    itemActive.classList.remove(className);

}


// click active header items
menuItems.forEach(item => {
    item.addEventListener('click', (event)=>{
        event.preventDefault();
        removeAvtiveClass('menu__item--active');
        item.classList.add('menu__item--active');
        
        const showContent = document.querySelector(`.${Object.entries(item.dataset)[0][1]}`);

        window.scrollTo({
            top: showContent.offsetTop - 120,
            behavior: "smooth",
        });
...
function observerHandler (allSection){
    allSection.map(sec =>{
        if (sec.isIntersecting){            
            removeAvtiveClass('menu__item--active');
            document.querySelector(`.menu__item[data-sections=${sec.target.className}]`).classList.add('menu__item--active');
        }
    });
}
